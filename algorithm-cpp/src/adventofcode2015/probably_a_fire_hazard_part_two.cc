...
int main(int argc, char const* argv[]) {
      ...
      std::ranges::for_each(matrix, [&grid, &action](const auto xy) {
        switch (action) {
          ...
          case action::turn_off:
            if (grid[xy.first][xy.second] > 0) grid[xy.first][xy.second] -= 1;
            break;
          case action::toggle:
  ...
  auto res = std::accumulate(std::cbegin(grid), std::cend(grid), 0,
                             [](const auto acc, const auto& row) {
                               return acc + std::accumulate(std::cbegin(row),
                                                            std::cend(row), 0);
                             });
